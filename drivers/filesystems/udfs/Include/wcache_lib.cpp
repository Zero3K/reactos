...
#define WCLOCK_RES   1
    ...
    _SEH2_TRY {
        ...
        if (!(Cache->tmp_buff = (PCHAR)MyAllocatePoolTag__(NonPagedPool, PacketSize*BlockSize, MEM_WCBUF_TAG))) {
            try_return(RC = STATUS_INSUFFICIENT_RESOURCES);
        }
        if (!(Cache->tmp_buff_r = (PCHAR)MyAllocatePoolTag__(NonPagedPool, PacketSize*BlockSize, MEM_WCBUF_TAG))) {
            try_return(RC = STATUS_INSUFFICIENT_RESOURCES);
        }
        if (!(Cache->reloc_tab = (PULONG)MyAllocatePoolTag__(NonPagedPool, Cache->PacketSize*sizeof(ULONG), MEM_WCBUF_TAG))) {
            try_return(RC = STATUS_INSUFFICIENT_RESOURCES);
        }
...
WCachePreReadPacket__(IN PIRP_CONTEXT IrpContext, IN PW_CACHE Cache, IN PVOID Context, IN lba_t Lba)
{
    ...
    // Skip already cached extent and validate sectors
    n = 0;
    while ((n < BCount) && (n < Cache->BlocksPerFrame)) {
        ...
        if (!(block_type & WCACHE_BLOCK_ZERO)) {
            zero = FALSE;
            if (!addr) {
                break; // Sector not cached, stop search
            }
        }
        n++;
    }

    // Read uncached sectors
    if (n < BCount) {
        // Try to read whole packet first
        if (!zero) {
        ...
        if (NT_SUCCESS(status)) {
            ...
            // Fallback: read sectors one by one
            err_count = 0;
            for (n = 0; n < BCount; n++, i++) {
                if (WCacheSectorAddr(block_array, i)) {
                    continue;
