...
)
{
    ...
    _SEH2_TRY {
        if (Ccb->Fcb) {
            UDFTouch(&(Ccb->Fcb->FcbNonpaged->FcbResource));
            UDFAcquireResourceExclusive(&(Ccb->Fcb->FcbNonpaged->FcbResource),TRUE);
            RemoveEntryList(&(Ccb->NextCCB));
            UDFReleaseResource(&(Ccb->Fcb->FcbNonpaged->FcbResource));
        } else {
            BrutePoint();
        }
